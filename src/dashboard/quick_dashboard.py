"""Smart Traffic Dashboard with live video, signals, and AI predictions."""
...
try:
    from src.dashboard.shared.styles import DASHBOARD_CSS, TRAFFIC_COLORS, DENSITY_COLORS, get_traffic_density_color
    from src.dashboard.shared.model_manager import TrainedModelManager
    from src.dashboard.components.session_tracker import SessionTracker
    from src.dashboard.utils.video_capture import create_robust_video_capture
    MODULES_AVAILABLE = True
except ImportError as e:
    print(f"Warning: Dashboard modules not available: {e}")
    MODULES_AVAILABLE = False
    # Fallback definitions if modules not available
    DASHBOARD_CSS = ""
    TRAFFIC_COLORS = {
        'RED': {'color': '#FF4444', 'bg_color': '#2D1B1B', 'text_color': '#FFFFFF', 'emoji': '🔴', 'icon': '⬛', 'pattern': 'solid'},
        'YELLOW': {'color': '#FFB84D', 'bg_color': '#2D2419', 'text_color': '#000000', 'emoji': '🟡', 'icon': '⬜', 'pattern': 'diagonal'},
        'GREEN': {'color': '#48BB78', 'bg_color': '#1A2E1A', 'text_color': '#FFFFFF', 'emoji': '🟢', 'icon': '⬜', 'pattern': 'dots'},
        'EMERGENCY': {'color': '#FF6B6B', 'bg_color': '#2D1A1A', 'text_color': '#FFFFFF', 'emoji': '🚨', 'icon': '⚠️', 'pattern': 'flash'}
    }
    DENSITY_COLORS = {
        'LOW': {'color': '#48BB78', 'bg_color': '#1A2E1A', 'text': 'Low Traffic', 'emoji': '🟢', 'threshold': 0.3},
        'MEDIUM': {'color': '#FFB84D', 'bg_color': '#2D2419', 'text': 'Medium Traffic', 'emoji': '🟡', 'threshold': 0.7},
        'HIGH': {'color': '#FF4444', 'bg_color': '#2D1B1B', 'text': 'High Traffic', 'emoji': '🔴', 'threshold': 1.0}
    }
    def get_traffic_density_color(density: float):
        if density <= 0.3: return DENSITY_COLORS['LOW']
        elif density <= 0.7: return DENSITY_COLORS['MEDIUM']
        else: return DENSITY_COLORS['HIGH']
...
if DASHBOARD_CSS:
    ...
    st.markdown("""
<style>
    .main-header { font-size: 2.5rem; color: #00D4FF; text-align: center; margin-bottom: 2rem; }
    .metric-card { background-color: #1E2329; padding: 1rem; border-radius: 0.5rem; border: 1px solid #2D3748; }
    .stApp { background-color: #0E1117; }
    [data-testid="metric-container"] { background-color: #1E2329; border: 1px solid #2D3748; padding: 1rem; border-radius: 0.5rem; }
</style>
...
# NOTE: The following classes are now imported from modules:
# - create_robust_video_capture: from src.dashboard.utils.video_capture
# - SessionTracker: from src.dashboard.components.session_tracker
# - TrainedModelManager: from src.dashboard.shared.model_manager
# 
# TrafficAnalysisReportGenerator is defined below (only class kept inline for now)

class TrafficAnalysisReportGenerator:
    """Generates comprehensive traffic analysis reports."""

    def __init__(self, session_tracker, model_manager):
    ...
    def generate_comprehensive_report(self):
        ...
        return {
            ...
            "speed_analysis": {
                "average_speed_kmh": round(np.mean(speeds), 1) if speeds else 0,
    ...
    def export_to_csv(self, report):
        ...
        csv_data = [['=== TMS2 TRAFFIC ANALYSIS REPORT ===']]
        csv_data.append(['Start Time', report.get('session_info', {}).get('start_time', '')])
        csv_data.append(['Duration (minutes)', report.get('session_info', {}).get('duration_minutes', '')])
        csv_data.append(['Total Vehicles', report.get('traffic_flow_analysis', {}).get('total_vehicles_detected', '')])
        output = io.StringIO()
    ...
    def export_to_json(self, report):
        ...
        return json.dumps({"tms2_traffic_report": report}, indent=2, default=str)
...
class LiveVideoStream:
    ...
    def start_stream(self, source: Union[str, int] = 0):
        """Start video stream from source with FFmpeg threading fixes."""
        try:
...
def main():
    ...
    if dashboard_mode == "Real-time Live Feed":
        ...
        if video_source_type == "Webcam":
            ...
            # Enhanced video dataset with Kaggle datasets
            st.sidebar.markdown("**📹 Video Dataset Options**")

            # Kaggle datasets
            kaggle_videos = []

            # Check for 720p road and traffic videos
            road_traffic_path = Path("data/kaggle/720p-road-and-traffic-video")
            if road_traffic_path.exists():
                for video_file in road_traffic_path.glob("*.mp4"):
                    kaggle_videos.append(str(video_file))

            # Check for highway traffic videos
            highway_path = Path("data/kaggle/highway-traffic-videos")
            if highway_path.exists():
                for video_file in highway_path.glob("*.mp4"):
                    kaggle_videos.append(str(video_file))

            # Original sample videos
            original_videos = [
                "data/videos/traffic_video_1.mp4",
                "data/videos/traffic_video_2.mp4",
                "data/videos/traffic_video_3.mp4",
                "data/videos/traffic_video_4.mp4",
                "data/videos/traffic_video_5.mp4",
                "data/videos/traffic_video_6.mp4",
                "data/videos/traffic_video_7.mp4",
                "data/videos/traffic_video_8.mp4",
                "data/videos/traffic_video_9.mp4",
                "data/videos/traffic_video_10.mp4"
            ]

            # Combine all available videos
            all_videos = []

            if kaggle_videos:
                st.sidebar.success(f"📊 Found {len(kaggle_videos)} Kaggle dataset videos")
                all_videos.extend(kaggle_videos)

            existing_original = [v for v in original_videos if Path(v).exists()]
            if existing_original:
                st.sidebar.info(f"📁 Found {len(existing_original)} original sample videos")
                all_videos.extend(existing_original)
...
# data/kaggle/highway-traffic-videos/
                ...
                all_videos = ["demo_mode"]

            video_source = st.sidebar.selectbox("Select Video Source", all_videos)
